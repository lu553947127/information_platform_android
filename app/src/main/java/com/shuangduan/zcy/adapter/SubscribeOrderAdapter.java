 ...
 * @Version: 1.0
 */
public class SubscribeOrderAdapter extends BaseQuickAdapter<SubscribeOrderBean.ListBean, BaseViewHolder> {
    public SubscribeOrderAdapter(int layoutResId, @Nullable List<SubscribeOrderBean.ListBean> data) {
    ...
    protected void convert(BaseViewHolder helper, SubscribeOrderBean.ListBean item) {
        helper.setText(R.id.tv_time, item.getSend_time())
                .setVisible(R.id.tv_is_view, item.getIs_view()==0)
                ...
                .setText(R.id.tv_number, "合计数量："+item.getTotalNumber()+"， 合计价格："+item.getTotalPrice());
